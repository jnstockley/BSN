    ...
    def __init__(self, channels: dict):
        ...
        for split_channels in split_channel_ids:
            ...
            for channel_data in channels_data:
                ...
                self.channel_file_repr[channel.channel_id] = {
                    "uploads": channel.current_upload_amount,
...
class YouTubeChannel:
    def __init__(self, channel_data: dict, previous_uploads: dict):
        ...
        if self.current_upload_amount > self.previous_upload_amount:
            logger.info(
                f"Upload amount changed for {self.channel_name} from {self.previous_upload_amount} to "
                f"{self.current_upload_amount}"
            )
            self.current_upload_id = self.get_upload_id()
            if self.current_upload_id != self.previous_upload_id:
                logger.info(
                    f"Upload ID changed for {self.channel_name} from {self.previous_upload_id} to "
    ...
    def __repr__(self):
        return (
            f"Channel ID: {self.channel_id}, Playlist ID: {self.playlist_id}, Previous Upload ID: "
    ...
    def get_upload_id(self) -> str:
        ...
        request: HttpRequest = youtube.playlistItems().list(
            part="contentDetails", playlistId=self.playlist_id, maxResults=1
        )
...
class YouTubeUpload:
    def __init__(self, upload_id: str):
        ...
        while self.tries < 3:
            ...
            self.uploaded_at = (
                parser.parse(upload_data["snippet"]["publishedAt"])
                .astimezone(utc)
                .replace(tzinfo=utc)
            )
        ...
        if self.tries != 3:
            ...
            if "maxres" in upload_data["snippet"]["thumbnails"]:
                self.thumbnail_url = upload_data["snippet"]["thumbnails"]["maxres"][
                    "url"
                ]
            else:
                self.thumbnail_url = upload_data["snippet"]["thumbnails"]["standard"][
                    "url"
                ]
    ...
    def __repr__(self):
        return (
            f"Upload ID: {self.upload_id}, Uploaded At: {self.uploaded_at}, Title: {self.title}, Length: "
