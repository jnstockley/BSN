...
class Notification:
    ...
    def create(self, **kwargs):
        if "starting_message" in kwargs and kwargs["starting_message"] is True:
            ...
            self.title = (
                f'{", ".join([channel.channel_name for channel in youtube_upload])} {verb}'
            ...
            self.title = (
                f'{", ".join([channel.channel_name for channel in youtube_short])} {verb}'
    ...
    def __setup_youtube_body__(youtube: list[YouTubeChannel]):
        if len(youtube) == 1:
            return (
                f"[{youtube[0].latest_upload.title}](https://youtube.com/watch?v="
    ...
    def send(self):
        self.apprise.notify(
            title=self.title, body=self.body, body_format=NotifyFormat.MARKDOWN
        )
